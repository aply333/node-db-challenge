...
        Relational Database, describes a genre of databases. The data being described has tables connecting/relating to each other.

        SQL a language, it fall under the catagory or Relational Database, but It is more specific in syntaxt and so on.
...
    Every data need to be able to be uniquly identified, that way we have means of apply CRUD operations on that data.
...
    We need an intermediate table to make the connections between the do, that way each connection can be uniqley identified.
